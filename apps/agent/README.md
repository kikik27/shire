...
Only these capabilities accept an override; every other capability inherits
the default chain:
...
`SHIRE_TEXT_API_KEY` and fails instead of reporting fixture usage when the LLM
...
Shared Postgres is configured with `SHIRE_AGENT_DATABASE_URL`. The web workspace
owns schema migrations; the agent owns matching evaluation, recommendation, and
agent run writes.
